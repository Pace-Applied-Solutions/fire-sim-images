...
import { DEFAULT_PROMPT_TEMPLATE, INTENSITY_VISUALS, TIME_OF_DAY_LIGHTING, FIRE_STAGE_DESCRIPTIONS, } from './promptTemplates.js';
/**
 ...
 */
function validatePromptSafety(promptText, safetySection) {
 ...
 */
function generateTerrainDescription(geoContext) {
 ...
 */
function generateNearbyFeatures(geoContext) {
 ...
 */
function determineSpreadDirection(windDirection) {
 ...
 */
function generateWindDescription(windSpeed, windDirection) {
    let strength = '';
    if (windSpeed < 10) {
        strength = 'light';
    }
 ...
 */
function calculateFireDimensions(perimeter) {
 ...
 */
function preparePromptData(request) {
    const { inputs, geoContext, perimeter } = request;
    if (!geoContext.vegetationType) {
        throw new Error(`Invalid geoContext: vegetationType is required. Received: ${JSON.stringify(geoContext)}`);
    }
    if (!INTENSITY_VISUALS[inputs.intensity]) {
        throw new Error(`Invalid intensity: "${inputs.intensity}". Valid values are: ${Object.keys(INTENSITY_VISUALS).join(', ')}`);
    }
    if (!FIRE_STAGE_DESCRIPTIONS[inputs.fireStage]) {
        throw new Error(`Invalid fireStage: "${inputs.fireStage}". Valid values are: spotFire, developing, established, major`);
    }
    const vegetationDescriptor = VEGETATION_DESCRIPTORS[geoContext.vegetationType] || geoContext.vegetationType.toLowerCase();
 ...
 */
function composePrompt(template, data, viewpoint) {
 ...
 */
export function generatePrompts(request, template = DEFAULT_PROMPT_TEMPLATE) {
    ...
    for (const viewpoint of request.requestedViews) {
        ...
        if (!validation.valid) {
            throw new Error(`Prompt contains blocked terms: ${validation.blockedTerms.join(', ')}. ` +
