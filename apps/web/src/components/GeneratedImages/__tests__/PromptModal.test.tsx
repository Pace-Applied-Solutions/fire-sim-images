...
describe('PromptModal', () => {
  ...
  it('renders with prompt text', () => {
    render(<PromptModal prompt={mockPrompt} viewpoint={mockViewpoint} onClose={mockOnClose} />);
  ...
  it('formats viewpoint names correctly', () => {
    ...
    testCases.forEach(({ viewpoint, expected }) => {
      const { unmount } = render(
        <PromptModal prompt={mockPrompt} viewpoint={viewpoint} onClose={mockOnClose} />
      );

      expect(
        screen.getByText(new RegExp(`Generation Prompt: ${expected}`, 'i'))
      ).toBeInTheDocument();
      unmount();
    });
  });
